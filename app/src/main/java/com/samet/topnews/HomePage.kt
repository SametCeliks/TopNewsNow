...
import com.samet.topnews.ui.theme.zendots
...
fun ArticleItem(article: Article,navController: NavHostController){
    Card(
        ...
        Row (
            ...
            AsyncImage(model = article.urlToImage?:R.drawable.fo,
...
fun CategoriesBar(newsViewModel: NewsViewModel){
    ...
    var isSearchExpander by remember{
    ...
    Row (
        ...
        if (isSearchExpander){
            OutlinedTextField(
                modifier = Modifier.padding(8.dp).height(50.dp).border(1.dp, Color.Gray, CircleShape)
                    .clip(CircleShape),
        ...
        categoriesList.forEach { category->
            Button( onClick = {
                //if you click, into categories - tiklandiginda kategorilerin içlerine girer
                newsViewModel.fetchTopHeaders(category)
            },   colors = ButtonColors(
                ...
                Text(text = category, fontFamily =  zendots, color = Color.White)
            }
        }
    }
}
